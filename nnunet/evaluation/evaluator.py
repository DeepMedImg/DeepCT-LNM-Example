...
def aggregate_classification_scores(test_ref_pairs,
                                     ...
                                     **metric_kwargs):
    all_scores = OrderedDict()
    for i, (pred, label, prob) in enumerate(test_ref_pairs):
        identifier = identifiers[i]
        case_score = OrderedDict()
        case_score['probability'] = float(prob) if prob is not None else prob
    ...
    preds = np.array([test_ref_pair[0] for test_ref_pair in test_ref_pairs if test_ref_pair[0] is not None])
    labels = np.array([test_ref_pair[1] for test_ref_pair in test_ref_pairs if test_ref_pair[0] is not None])
    probs = np.array([test_ref_pair[2] for test_ref_pair in test_ref_pairs if test_ref_pair[0] is not None])
