...
def get_case_identifiers_npy(folder):
...
class DataLoader3D_Balanced(DataLoader3D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        super().__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
                 oversample_foreground_percent, memmap_mode, pad_mode, pad_kwargs_data, pad_sides)
        self.list_of_cls_target_pos = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] > 0]
        self.list_of_cls_target_neg = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] == 0]

    def generate_train_batch(self):
        # selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        selected_keys = np.concatenate([np.random.choice(self.list_of_cls_target_pos, self.batch_size // 2, True, None),
                                        np.random.choice(self.list_of_cls_target_neg, self.batch_size // 2, True, None)])
...
class DataLoader3D_Vessel(DataLoader3D):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None):
        super().__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
                 ...
                 pad_sides)
        task407_split_pkl = '/data86/pancreaticCancer/nnUNETFrame/nnUNET_DATASET/nnUNET_preprocessed/Task407_Arterial_Venous_organ_vessel_supervised_seg/splits_final.pkl'
        with open(task407_split_pkl, 'rb') as f:
            split_dict = pickle.load(f)
        self.vessel_20_complete_labeled_keys = split_dict[0]['train']

    def generate_train_batch(self):
        selected_keys = np.concatenate((np.random.choice(self.list_of_keys, self.batch_size // 2, True, None),
                                       np.random.choice(self.vessel_20_complete_labeled_keys, self.batch_size // 2, True, None)))
        data = []
        seg = []
        case_properties = []
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    ...
                    p = [1. / np.sqrt(np.sum(case_all_data[-1] == foreground_class)) for foreground_class in foreground_classes]
                    p_norm = [p1 / np.sum(p) for p1 in p]
                    selected_class = np.random.choice(foreground_classes, p=p_norm)
                ...
                if len(voxels_of_that_class) != 0:
                    ...
                    # not center voxel
                    bbox_x_lb = max(lb_x, selected_voxel[0] - np.random.randint(15, self.patch_size[0] - 15))
                    bbox_y_lb = max(lb_y, selected_voxel[1] - np.random.randint(15, self.patch_size[1] - 15))
                    bbox_z_lb = max(lb_z, selected_voxel[2] - np.random.randint(15, self.patch_size[2] - 15))
                else:
                    # If the selected class is indeed not present then we fall back to random cropping. We can do that
                    # because this case is extremely rare.
                    bbox_x_lb = np.random.randint(lb_x, ub_x + 1)
...
class DataLoader3DV2(DataLoader3D):
    '''
    'selected voxel is center voxel' --> 'selected voxel is random voxel'
    '''
    def generate_train_batch(self):
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        data = []
...
class DataLoader3DV2_AttMap(DataLoader3D):
    '''
    'selected voxel is center voxel' --> 'selected voxel is random voxel'
    Additional channel: attention map
    '''
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                ...
                # bbox_y_lb = np.random.randint(lb_y, ub_y + 1)
                # bbox_z_lb = np.random.randint(lb_z, ub_z + 1)
                # select voxels that attmap > 0
                # voxels_of_that_class = np.argwhere(case_all_data[-1] > 1e-2)
                corner_min = self._data[i]['properties']['voxels_of_positive_attention']['corner_min']
                corner_max = self._data[i]['properties']['voxels_of_positive_attention']['corner_max']
                bbox_x_lb = np.random.randint(low=min(corner_min[0], corner_max[0] - self.patch_size[0]), high=max(corner_min[0], corner_max[0] - self.patch_size[0])+1)
                bbox_y_lb = np.random.randint(low=min(corner_min[1], corner_max[1] - self.patch_size[1]), high=max(corner_min[1], corner_max[1] - self.patch_size[1])+1)
                bbox_z_lb = np.random.randint(low=min(corner_min[2], corner_max[2] - self.patch_size[2]), high=max(corner_min[2], corner_max[2] - self.patch_size[2])+1)
            else:
                # this saves us a np.unique. Preprocessing already did that for all cases. Neat.
                foreground_classes = np.array(self._data[i]['properties']['classes'])
                foreground_classes = foreground_classes[foreground_classes > 0]
                ...
                # we could precompute that to save CPU time but I am too lazy right now
                # the last channel is attmap
                # voxels_of_that_class = np.argwhere(case_all_data[-2] == selected_class)
                voxels_of_that_class = self._data[i]['properties']['voxels_of_foreground_classes'][selected_class]
        ...
        # time2 = time()
        # print('dataset loading took %.2f s, keys: %s, %s' % ((time2 - time1), selected_keys[0], selected_keys[1]))

        return {'data':data, 'seg':seg, 'properties':case_properties, 'keys': selected_keys}

class DataLoader3D_Classification(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3D_Classification, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
        ...
        try:
            ...
            self.patient_level = False
        except:
            self.list_of_cls_target_1 = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] != 0]
            self.list_of_cls_target_0 = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] == 0]
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.patient_level:
                cls_target.append(int(bool(self._data[i]['properties']['metastasis_stage'])))
            else:
                cls_target.append(self._data[i]['properties']['cls_target'] - 1)
            # cases are stores as npz, but we require unpack_dataset to be run. This will decompress them into npy
            # which is much faster to access
            if isfile(self._data[i]['data_file'][:-4] + ".npy"):
                case_all_data = np.load(self._data[i]['data_file'][:-4] + ".npy", self.memmap_mode)
        ...
        return {'data':data, 'seg':seg, 'properties':case_properties, 'keys': selected_keys, 'cls_target':cls_target}
...
class DataLoader2DClassification(DataLoader2D):
    '''
    Lymph node metastasis classification from PDAC
    '''
    def __init__(self, data, patch_size, final_patch_size, batch_size, transpose=None,
                 oversample_foreground_percent=0.0, memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None):
        super(DataLoader2DClassification, self).__init__(data, patch_size, final_patch_size, batch_size, transpose, oversample_foreground_percent,
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            cls_targets.append(int(bool(properties['metastasis_stage'])))

            if self.transpose is not None:
                leading_axes = [self.transpose[0], self.transpose[1], self.transpose[2]]
            else:
                leading_axes = [0, 1, 2]

            leading_axis = np.random.choice(leading_axes)
            # leading_axis = 0
            # select one class, then select a slice that contains that class
            classes_in_slice_per_axis = properties.get("classes_in_slice_per_axis")
            possible_classes = np.unique(properties['classes'])
            possible_classes = possible_classes[possible_classes > 0]
            assert len(possible_classes) > 0 and not (0 in possible_classes.shape)
            selected_class = np.random.choice(possible_classes)
            leading_seg_idx = np.random.choice(
                list(properties.get("classes_in_slice_per_axis")[leading_axis][selected_class].keys()))

            valid_slices = classes_in_slice_per_axis[leading_axis][selected_class][leading_seg_idx]
            assert len(valid_slices) != 0
            random_slice = np.random.choice(valid_slices)

            cropped_path = join(os.path.dirname(self._data[i]['data_file']), 'cropped_slice', os.path.basename(self._data[i]['data_file'][:-4])
                                + '_axis%d_cls%d_segidx%d_slice%d.npy'%(leading_axis, selected_class, leading_seg_idx, random_slice))
            if os.path.exists(cropped_path):
                case_all_data = np.load(cropped_path)[None]
                num_phase = 2
                if not np.all(case_all_data.shape[2:] == self.patch_size):
                    # center crop
                    center_voxel = [case_all_data.shape[2] //2, case_all_data.shape[3] // 2]
                    case_all_data = case_all_data[:, :, center_voxel[0] - self.patch_size[0] //2:center_voxel[0] + self.patch_size[0] //2 + self.patch_size[0] %2,
                                    center_voxel[1] - self.patch_size[1] //2:center_voxel[1] + self.patch_size[1] //2 + self.patch_size[1] %2]
                ...
                #             np.vstack((np.array(case_all_data.shape[1:])[None], np.array(self.patch_size)[None])), 0)
                # if new_shp is not None:
                #     case_all_data_donly = pad_nd_image(case_all_data[:num_phase], new_shp, self.pad_mode, kwargs=self.pad_kwargs_data)
                #     case_all_data_segnonly = pad_nd_image(case_all_data[num_phase:][leading_seg_idx: leading_seg_idx+1], new_shp, 'constant', kwargs={'constant_values':-1})
                #     case_all_data = np.vstack((case_all_data_donly, case_all_data_segnonly))[None]
                # else:
                #     case_all_data = np.vstack((case_all_data[:num_phase], case_all_data[num_phase:][leading_seg_idx: leading_seg_idx+1]))[None]
                #
                # case_all_data = crop_2D_image_force_fg(case_all_data[0], tuple(self.patch_size), selected_class)[None]
            data.append(case_all_data[:, :num_phase])
            seg.append(case_all_data[:, num_phase:])
        data = np.vstack(data)
        seg = np.vstack(seg)
        cls_targets = np.array(cls_targets)
        keys = selected_keys
        return {'data':data, 'seg':seg, 'properties':case_properties, "keys": keys, 'cls_target':cls_targets}


class DataLoader2DClassification_perFold(DataLoader2D):
    '''
    Lymph node metastasis classification from PDAC using per-fold predicted PDAC mask
    '''
    def __init__(self, data, patch_size, final_patch_size, batch_size, transpose=None,
                 oversample_foreground_percent=0.0, memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None, fold=0):
        super(DataLoader2DClassification_perFold, self).__init__(data, patch_size, final_patch_size, batch_size, transpose, oversample_foreground_percent,
                                                         memmap_mode, pseudo_3d_slices, pad_mode, pad_kwargs_data, pad_sides)
        valid_keys = set([filename[:8] for filename in os.listdir(join(os.path.dirname(self._data[list(self._data.keys())[0]]['data_file']), 'cropped_slice')) if 'fold%d.npy'%fold in filename])
        self.data_keys = sorted(list(set(self._data.keys()) & valid_keys))
        self.list_of_cls_target_pos = [key for key in self.data_keys if self._data[key]['properties']['metastasis_stage'] >0]
        self.list_of_cls_target_neg = [key for key in self.data_keys if self._data[key]['properties']['metastasis_stage'] == 0]
        self.fold = fold
        # self.key_idx=0
    ...
    def generate_train_batch(self):
        selected_keys = np.concatenate([np.random.choice(self.list_of_cls_target_pos, self.batch_size // 2, True, None),
             np.random.choice(self.list_of_cls_target_neg, self.batch_size // 2, True, None)])
        # if self.key_idx + self.batch_size < len(self.data_keys):
        #     selected_keys = np.array(self.data_keys[self.key_idx:self.key_idx+self.batch_size])
        # else:
        #     selected_keys = np.concatenate([np.array(self.data_keys[self.key_idx:]), np.array(self.data_keys[:self.key_idx+self.batch_size -len(self.data_keys)])])
        # self.key_idx = (self.key_idx + self.batch_size) % len(self.data_keys)
        data = []
        seg = []
        case_properties = []
        cls_targets = []
        for j, i in enumerate(selected_keys):
            ...
            # leading_axis = 0
            # select one class, then select a slice that contains that class
            classes_in_slice_per_axis = properties.get("classes_in_slice_per_axis_fold%d" % self.fold)
            possible_classes = np.unique(properties['classes'])
            possible_classes = possible_classes[possible_classes > 0]
            assert len(possible_classes) > 0 and not (0 in possible_classes.shape)
            selected_class = np.random.choice(possible_classes)
            leading_seg_idx = np.random.choice(
                list(properties.get("classes_in_slice_per_axis_fold%d" % self.fold)[leading_axis][selected_class].keys()))
            ...
            cropped_path = join(os.path.dirname(self._data[i]['data_file']), 'cropped_slice', os.path.basename(self._data[i]['data_file'][:-4])
                                + '_axis%d_cls%d_segidx%d_slice%d_fold%d.npy'%(leading_axis, selected_class, leading_seg_idx, random_slice, self.fold))
            if os.path.exists(cropped_path):
                case_all_data = np.load(cropped_path)[None]
                num_phase = 2
                if not np.all(case_all_data.shape[2:] == self.patch_size):
...
class DataLoader2DClassification_perFold_wLabel(DataLoader2DClassification_perFold):
    '''
        Lymph node metastasis classification from PDAC using per-fold predicted PDAC mask and annotation
        '''

    def __init__(self, data, patch_size, final_patch_size, batch_size, transpose=None,
                 oversample_foreground_percent=0.0, memmap_mode="r", pseudo_3d_slices=1, pad_mode="edge",
                 pad_kwargs_data=None, pad_sides=None, fold=0, train_or_val = 'train', dice_threshold=0):
        super(DataLoader2DClassification_perFold, self).__init__(data, patch_size, final_patch_size, batch_size,
                                                                 transpose, oversample_foreground_percent,
                                                                 memmap_mode, pseudo_3d_slices, pad_mode,
                                                                 pad_kwargs_data, pad_sides)
        valid_keys = set([filename[:8] for filename in os.listdir(join(os.path.dirname(self._data[list(self._data.keys())[0]]['data_file']), 'cropped_slice')) if
                          'fold%d.npy' % fold in filename])
        self.data_keys = sorted(list(set(self._data.keys()) & valid_keys))
        self.list_of_cls_target_pos = [key for key in self.data_keys if self._data[key]['properties']['metastasis_stage'] > 0
                                       and self._data[key]['properties']['PDAC_dice_fold%d'%fold] > dice_threshold]
        self.list_of_cls_target_neg = [key for key in self.data_keys if self._data[key]['properties']['metastasis_stage'] == 0
                                       and self._data[key]['properties']['PDAC_dice_fold%d'%fold] > dice_threshold]
        self.list_of_cls_target_pos_label = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] > 0]
        self.list_of_cls_target_neg_label = [key for key in self._data.keys() if self._data[key]['properties']['metastasis_stage'] == 0]
        self.fold = fold
        self.train_or_val=train_or_val

    def generate_train_batch(self):
        if self.train_or_val == 'train':
            selected_keys = np.concatenate([np.random.choice(self.list_of_cls_target_pos, self.batch_size // 4, True, None),
                                            np.random.choice(self.list_of_cls_target_neg, self.batch_size // 4, True, None),
                                            np.random.choice(self.list_of_cls_target_pos_label, self.batch_size // 4, True, None),
                                            np.random.choice(self.list_of_cls_target_neg_label, self.batch_size // 4, True, None)])
        else:
            selected_keys = np.concatenate([np.random.choice(self.list_of_cls_target_pos, self.batch_size // 4 *2, True, None),
                                            np.random.choice(self.list_of_cls_target_neg, self.batch_size // 4 *2, True, None)])
        # if self.key_idx + self.batch_size < len(self.data_keys):
        #     selected_keys = np.array(self.data_keys[self.key_idx:self.key_idx+self.batch_size])
        # else:
        #     selected_keys = np.concatenate([np.array(self.data_keys[self.key_idx:]), np.array(self.data_keys[:self.key_idx+self.batch_size -len(self.data_keys)])])
        ...
        for j, i in enumerate(selected_keys):
            ...
            # leading_axis = 0
            # select one class, then select a slice that contains that class
            if (self.train_or_val == 'train' and j >= self.batch_size // 4 * 2):
                classes_in_slice_per_axis = properties.get("classes_in_slice_per_axis")
                possible_classes = np.unique(properties['classes'])
                possible_classes = possible_classes[possible_classes > 0]
                assert len(possible_classes) > 0 and not (0 in possible_classes.shape)
                ...
                selected_class = np.random.choice(possible_classes)
                leading_seg_idx = np.random.choice(
                    list(properties.get("classes_in_slice_per_axis_fold%d" % self.fold)[leading_axis][
                             selected_class].keys()))
            ...
            if os.path.exists(cropped_path):
                ...
                if not np.all(case_all_data.shape[2:] == self.patch_size):
                    ...
                    case_all_data = case_all_data[:, :,
                                    center_voxel[0] - self.patch_size[0] // 2:center_voxel[0] + self.patch_size[
                                        0] // 2 + self.patch_size[0] % 2,
                                    center_voxel[1] - self.patch_size[1] // 2:center_voxel[1] + self.patch_size[
                                        1] // 2 + self.patch_size[1] % 2]
