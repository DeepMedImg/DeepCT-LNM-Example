...
from nnunet.training.data_augmentation.custom_transforms import Convert3DTo2DTransform, Convert2DTo3DTransform, \
    MaskTransform
from nnunet.training.data_augmentation.downsampling import DownsampleSegForDSTransform3, DownsampleSegForDSTransform2, DownsampleDataForDSTransform2
from nnunet.training.data_augmentation.pyramid_augmentations import MoveSegAsOneHotToData, \
    ApplyRandomBinaryOperatorTransform, \
    RemoveRandomConnectedComponentFromOneHotEncodingTransform
import os

from nnunet.training.data_augmentation.my_custom_transform import MoveSDMToData, MoveSDMOutOfData, SelectiveRemoveLabelTransform, MoveKeyOut, \
    SpatialTransformAttmap, GenerateJitteringBBox, MaskTransformV2, MaskTransformV3, Mask2Box, Mask2JitteringBox
...
def get_moreDA_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    if params.get("has_sdm") is not None and params.get("has_sdm"):
        tr_transforms.append(MoveSDMOutOfData())
    ...
    if params.get("has_sdm") is not None and params.get("has_sdm"):
        tr_transforms.append(MoveSDMToData())
    ...
    if params.get("move_seg_chanel_to_data") is not None and params.get("move_seg_chanel_to_data"):
        tr_transforms.append(MoveSDMToData(key_origin='seg', key_target='data', remove_from_origin=False, binarize=True))
    ...
    if params.get("move_seg_chanel_to_data") is not None and params.get("move_seg_chanel_to_data"):
        val_transforms.append(MoveSDMToData(key_origin='seg', key_target='data', remove_from_origin=False, binarize=True))
...
def get_moreDA_augmentation_attmap(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
                            seeds_train=None, seeds_val=None, order_seg=1, order_data=3, deep_supervision_scales=None,
                            soft_ds=False,
                            classes=None, pin_memory=True):
    ...
    tr_transforms.append(SpatialTransformAttmap(
        patch_size, patch_center_dist_from_border=None,
        do_elastic_deform=params.get("do_elastic"), alpha=params.get("elastic_deform_alpha"),
        sigma=params.get("elastic_deform_sigma"),
        do_rotation=params.get("do_rotation"), angle_x=params.get("rotation_x"), angle_y=params.get("rotation_y"),
    ...
    tr_transforms.append(MoveKeyOut(key_origin="seg", key_target="attmap", channel=-1))
    tr_transforms.append(RenameTransform('seg', 'target', True))
    ...
    if deep_supervision_scales is not None:
        if soft_ds:
            ...
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, 0, input_key='target',
                                                              output_key='target'))
            tr_transforms.append(DownsampleDataForDSTransform2(deep_supervision_scales, 3, 0, input_key='attmap',
                                                              output_key='attmap'))

    tr_transforms.append(NumpyToTensor(['data', 'target', 'attmap'], 'float'))
    tr_transforms = Compose(tr_transforms)
    ...
    val_transforms.append(MoveKeyOut(key_origin="seg", key_target="attmap", channel=-1))
    val_transforms.append(RenameTransform('seg', 'target', True))
    ...
    if deep_supervision_scales is not None:
        if soft_ds:
            ...
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, 0, input_key='target',
                                                               output_key='target'))
            val_transforms.append(DownsampleDataForDSTransform2(deep_supervision_scales, 3, 0, input_key='attmap',
                                                              output_key='attmap'))

    val_transforms.append(NumpyToTensor(['data', 'target', 'attmap'], 'float'))
    val_transforms = Compose(val_transforms)
...
def get_moreDA_augmentation_vessel_semi(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
                            seeds_train=None, seeds_val=None, order_seg=1, order_data=3, deep_supervision_scales=None,
                            soft_ds=False,
                            classes=None, pin_memory=True):
    ...
    tr_transforms.append(RemoveLabelTransform(-1, 0))
    tr_transforms.append(SelectiveRemoveLabelTransform(remove_label=0, replace_with=-1, foreground_num=params['foreground_num']))
...
def get_moreDA_augmentation_classification(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
                            seeds_train=None, seeds_val=None, order_seg=1, order_data=3, deep_supervision_scales=None,
                            soft_ds=False,
                            classes=None, pin_memory=True):
    ...
    if params.get("GenerateBBox") is not None and params.get("GenerateBBox"):
        tr_transforms.append(GenerateJitteringBBox(margins_min=params.get('margins_min'), margins_max=params.get('margins_max')))

    if params.get("mask_out_of_box_to_zero") is not None and params.get("mask_out_of_box_to_zero"):
        tr_transforms.append(MaskTransformV2(mask_idx_in_seg=0, set_outside_to=0))

    if params.get("mask_out_of_mask_to_zero") is not None and params.get("mask_out_of_mask_to_zero"):
        tr_transforms.append(MaskTransformV3(mask_idx_in_seg=0, set_outside_to=0))

    if params.get("change_mask_to_box") is not None and params.get("change_mask_to_box"):
        tr_transforms.append(Mask2Box(mask_idx_in_seg=0))

    if params.get("change_jittering_mask_to_box") is not None and params.get("change_jittering_mask_to_box"):
        tr_transforms.append(Mask2JitteringBox(mask_idx_in_seg=0))

    tr_transforms.append(RemoveLabelTransform(-1, 0))

    if params.get("move_seg_chanel_to_data") is not None and params.get("move_seg_chanel_to_data"):
        tr_transforms.append(MoveSDMToData(key_origin='seg', key_target='data', remove_from_origin=params.get('remove_seg_from_data', True)))
        # if params.get("cascade_do_cascade_augmentations") is not None and params.get(
        #         "cascade_do_cascade_augmentations"):
        #     if params.get("cascade_random_binary_transform_p") > 0:
        #         tr_transforms.append(ApplyRandomBinaryOperatorTransform(
    ...
    #                                                           output_key='target'))
    # tr_transforms.append(NumpyToTensor(['data', 'target'], 'float'))
    tr_transforms.append(NumpyToTensor(['data', 'roi', 'cls_target'], 'float'))
    if params.get("move_seg_chanel_to_data") is None or not params.get('remove_seg_from_data', True):
        tr_transforms.append(NumpyToTensor(['seg'], 'float'))
    tr_transforms = Compose(tr_transforms)
    ...
    if params.get("GenerateBBox") is not None:
        val_transforms.append(GenerateJitteringBBox(margins_min=np.add(params.get('margins_min'), params.get('margins_max'))/2.0, margins_max=np.add(params.get('margins_min'), params.get('margins_max'))/2.0))

    if params.get("mask_out_of_box_to_zero") is not None and params.get("mask_out_of_box_to_zero"):
        val_transforms.append(MaskTransformV2(mask_idx_in_seg=0, set_outside_to=0))

    if params.get("mask_out_of_mask_to_zero") is not None and params.get("mask_out_of_mask_to_zero"):
        val_transforms.append(MaskTransformV3(mask_idx_in_seg=0, set_outside_to=0))

    if params.get("change_mask_to_box") is not None and params.get("change_mask_to_box"):
        val_transforms.append(Mask2Box(mask_idx_in_seg=0))

    if params.get("change_jittering_mask_to_box") is not None and params.get("change_jittering_mask_to_box"):
        val_transforms.append(Mask2JitteringBox(mask_idx_in_seg=0))

    if params.get("move_seg_chanel_to_data") is not None and params.get("move_seg_chanel_to_data"):
        val_transforms.append(MoveSDMToData(key_origin='seg', key_target='data', remove_from_origin=params.get('remove_seg_from_data', True)))
    ...
    #
    # val_transforms.append(NumpyToTensor(['data', 'target'], 'float'))
    val_transforms.append(NumpyToTensor(['data', 'roi', 'cls_target'], 'float'))
    if params.get("move_seg_chanel_to_data") is None or not params.get('remove_seg_from_data', True):
        val_transforms.append(NumpyToTensor(['seg'], 'float'))
    val_transforms = Compose(val_transforms)
