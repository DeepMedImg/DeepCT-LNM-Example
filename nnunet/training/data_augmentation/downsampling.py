...
class DownsampleDataForDSTransform2(AbstractTransform):
    '''
    data_dict['output_key'] will be a list of data scaled according to ds_scales
    '''
    def __init__(self, ds_scales=(1, 0.5, 0.25), order=0, cval=0, input_key="seg", output_key="seg", axes=None):
        self.axes = axes
        self.output_key = output_key
    ...
    def __call__(self, **data_dict):
        data_dict[self.output_key] = downsample_data_for_ds_transform2(data_dict[self.input_key], self.ds_scales, self.order,
                                                           self.cval, self.axes)
        return data_dict

def downsample_data_for_ds_transform2(data, ds_scales=((1, 1, 1), (0.5, 0.5, 0.5), (0.25, 0.25, 0.25)), order=0, cval=0, axes=None):
    ...
    for s in ds_scales:
        if all([i == 1 for i in s]):
            ...
            for i, a in enumerate(axes):
                new_shape[a] *= s[i]
            new_shape = np.round(new_shape).astype(int)
            out_data = np.zeros(new_shape, dtype=data.dtype)
            for b in range(data.shape[0]):
                for c in range(data.shape[1]):
                    out_data[b, c] = resize(data[b, c], new_shape[2:], order, cval=cval, **kwargs)
