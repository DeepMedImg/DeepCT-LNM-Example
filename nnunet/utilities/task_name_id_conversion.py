...
def convert_id_to_task_name(task_id: int):
    ...
    if network_training_output_dir is not None:
        candidates_trained = subdirs(join(network_training_output_dir, '2d'), prefix=startswith, join=False) + \
                             subdirs(join(network_training_output_dir, '3d_fullres'), prefix=startswith, join=False)
    else:
        candidates_trained = []


    all_candidates = candidates_cropped + candidates_preprocessed + candidates_raw + candidates_trained
