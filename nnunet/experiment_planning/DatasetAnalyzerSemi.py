...
class DatasetAnalyzer(object):
    def __init__(self, folder_with_cropped_data, overwrite=True, num_processes=default_num_threads):
        ...
        self.labeled_patient_identifiers = get_patient_identifiers_from_cropped_files(join(self.folder_with_cropped_data, 'labeled'))
        self.unlabeled_patient_identifiers = get_patient_identifiers_from_cropped_files(join(self.folder_with_cropped_data, 'unlabeled'))
    ...
    def load_properties_of_cropped(self, subfolder, case_identifier):
        with open(join(self.folder_with_cropped_data, subfolder, "%s.pkl" % case_identifier), 'rb') as f:
    ...
    def _get_unique_labels(self, patient_identifier):
        seg = np.load(join(self.folder_with_cropped_data, 'labeled', patient_identifier) + ".npz")['data'][-1]
    ...
    def analyse_segmentations(self):
        ...
        if self.overwrite or not isfile(self.props_per_case_file):
            ...
            res = p.map(self._get_unique_labels, self.labeled_patient_identifiers)
            p.close()
            p.join()
            ...
            for p, unique_classes in \
                            zip(self.labeled_patient_identifiers, res):
                props = dict()
    ...
    def get_sizes_and_spacings_after_cropping(self):
        ...
        for subfolder in ['labeled', 'unlabeled']:
            case_identifiers = get_patient_identifiers_from_cropped_files(join(self.folder_with_cropped_data,subfolder))
            sizes = []
            spacings = []
            for c in case_identifiers:
                properties = self.load_properties_of_cropped(subfolder, c)
                sizes.append(properties["size_after_cropping"])
                spacings.append(properties["original_spacing"])
            all_sizes.extend(sizes)
            all_spacings.extend(spacings)
        return all_sizes, all_spacings
    ...
    def get_size_reduction_by_cropping(self):
        ...
        for subfolder, patient_identifiers in zip(['labeled', 'unlabeled'], [self.labeled_patient_identifiers, self.unlabeled_patient_identifiers]):
            for p in patient_identifiers:
                props = self.load_properties_of_cropped(subfolder, p)
    ...
    def _get_voxels_in_foreground(self, patient_identifier, modality_id):
        all_data = np.load(join(self.folder_with_cropped_data, 'labeled', patient_identifier) + ".npz")['data']
        modality = all_data[modality_id]
        mask = all_data[-1] > 0
        voxels = list(modality[mask][::2]) # no need to take every voxel 10
        return voxels
    ...
    def collect_intensity_properties(self, num_modalities):
        if self.overwrite or not isfile(self.intensityproperties_file):
            ...
            for mod_id in range(num_modalities):
                ...
                v = p.starmap(self._get_voxels_in_foreground, zip(self.labeled_patient_identifiers,
                                                              [mod_id] * len(self.labeled_patient_identifiers)))
                ...
                for i, pat in enumerate(self.labeled_patient_identifiers):
