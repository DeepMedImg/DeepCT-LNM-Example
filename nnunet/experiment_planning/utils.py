...
def create_lists_from_splitted_dataset(base_folder_splitted):
    ...
    for tr in training_files:
        cur_pat = []
        for mod in d['modality'].keys():
            cur_pat.append(join(base_folder_splitted, "imagesTr", tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % int(mod)))
        cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'].split("/")[-1]))
        lists.append(cur_pat)
    return lists, {int(i): d['modality'][str(i)] for i in d['modality'].keys()}

def create_lists_from_splitted_dataset_with_inferTr(base_folder_splitted):
    lists = []
    ...
    for tr in training_files:
        ...
        for mod in d['modality'].keys():
            cur_pat.append(join(base_folder_splitted, "imagesTr", tr['image'].split("/")[-1][:-7] +
                                "_%04.0d.nii.gz" % int(mod)))
        cur_pat.append(join(base_folder_splitted, "labelsTr", tr['label'].split("/")[-1]))
        cur_pat.append(join(base_folder_splitted, "inferTr", tr['infer'].split("/")[-1]))

        lists.append(cur_pat)
    return lists, {int(i): d['modality'][str(i)] for i in d['modality'].keys()}

def create_lists_from_splitted_dataset_without_json(base_folder_splitted):
    lists = []
    training_files = [filename for filename in os.listdir(os.path.join(base_folder_splitted, "PDAC")) if filename.endswith('.nii.gz')]
    modality = {'1': 'CT', '3':'CT'}
    for tr in training_files:
        cur_pat = []
        for mod in modality.keys():
            cur_pat.append(join(base_folder_splitted, "image", tr[:-7] + "_%04.0d.nii.gz" % int(mod)))
        cur_pat.append(join(base_folder_splitted, "PDAC", tr))

        lists.append(cur_pat)
    return lists, {int(i): modality[str(i)] for i in modality.keys()}
...
def crop_pdac_classification(raw_data_dir, cropped_out_dir, override=False, num_threads=default_num_threads):
    maybe_mkdir_p(cropped_out_dir)

    if override and isdir(cropped_out_dir):
        shutil.rmtree(cropped_out_dir)
        maybe_mkdir_p(cropped_out_dir)
    lists, _ = create_lists_from_splitted_dataset_without_json(raw_data_dir)
